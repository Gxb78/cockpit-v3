...
### BUG-20260611-01 - [RESOLU] Pagination klines sans startTime plafonnee a 1000 bougies malgre `_KLINES_MAX_LIMIT = 5000`

- Symptome: Le graphe prix ne chargeait jamais plus de ~1000 bougies (1m) meme apres la limite passee a 5000 dans BUG-20260607-01, et le panneau CVD (retention live jusqu'a 200k points) pouvait paraitre avoir "plus d'historique" que le prix.
- Cause racine: Dans `fetch_klines` (`app_parts/23_routes_market.py`), quand `start_time` est `None`, l'appel Binance `/api/v3/klines` (sans `startTime`) renvoie les **plus recentes** `MAX_PER_REQUEST` (1000) bougies. La pagination suivante calcule `current_start = batch[-1][0] + interval_ms`, c'est-a-dire un timestamp dans le **futur** (juste apres "maintenant") — la page suivante revient donc vide et la boucle s'arrete avec seulement 1000 bougies, quel que soit `limit`.
- Regle de prevention: Pour toute pagination Binance/Hyperliquid sans `startTime` explicite et `limit > MAX_PER_REQUEST`, ancrer `start_time = now_ms - limit * interval_ms` **avant** la boucle de pagination, pour que celle-ci avance bien vers le present au lieu de partir du futur. Cote frontend, un backfill "deep history" one-shot par `(source, symbol, interval)` (`fetchDeepHistory` dans `078_v6_local_engine_client.js`) fusionne ces bougies REST plus anciennes en tete de `chartCandles`/`_candlesByInterval` (dedup par `openTime`), sans toucher au flux live WS.
- Test de non-regression: `tests/test_playbook_lessons_guardrails.py` (cette verif); verifier `curl /api/market/klines?symbol=BTCUSDT&interval=1m&limit=5000&soft=1&force=1` renvoie bien ~5000 candles couvrant plusieurs jours, et que `store.getState().chartCandles.length` atteint 5000 apres `connect()`.
- Fichiers a surveiller: `app_parts/23_routes_market.py` (`fetch_klines`), `static/js/split/078_v6_local_engine_client.js` (`restKlinesUrl`, `fetchDeepHistory`, `mergeDeepHistory`).

### LESSON-20260611-05 - Deep history x10 (50k bougies) : Binance pagination longue + pagination Hyperliquid candleSnapshot

- Contexte: Suite a BUG-20260611-01, l'utilisateur a demande 10x plus d'historique (50000 bougies au lieu de 5000, soit ~34.7 jours en 1m).
- Regle: `_KLINES_MAX_LIMIT` (`app_parts/23_routes_market.py`) et `DEEP_HISTORY_LIMIT` (`078_v6_local_engine_client.js`) sont passes de 5000 a 50000. La pagination Binance existante (ancrage `start_time = now - limit*interval_ms` de BUG-20260611-01) gere nativement les ~50 pages necessaires en 1m — elle s'arrete naturellement plus tot pour les intervalles ou moins de bougies existent (`len(batch) < fetch`).
- Regle de prevention: Pour une pagination longue (>10 pages), ne JAMAIS jeter les bougies deja collectees si une page intermediaire echoue — `fetch_klines` retourne maintenant les `all_raw` deja accumules (avec `upstream_error` renseigne) plutot que de tomber sur le cache stale/vide. Idem cote Hyperliquid : `hyperliquid_klines` (`app_parts/25_routes_hyperliquid.py`) pagine desormais via `candleSnapshot` en avancant `startTime = batch[-1].openTime + interval_ms` (meme patron que Binance), plafonne a `_HL_MAX_KLINE_PAGES = 60` pages et `_HL_KLINES_MAX_LIMIT = 50000`, dedupe par `openTime` dans un dict.
- Test de non-regression: `tests/test_hyperliquid_routes.py::test_klines_normalizes_builder_prefixed_contract` (pagination doit toujours s'arreter correctement quand `current_start >= end_time`); verifier `curl /api/market/klines?symbol=BTCUSDT&interval=1m&limit=50000&soft=1&force=1` couvre ~35 jours, et `chartCandlesCount` apres `connect()`.
- Fichiers a surveiller: `app_parts/23_routes_market.py` (`fetch_klines`, `_KLINES_MAX_LIMIT`), `app_parts/25_routes_hyperliquid.py` (`hyperliquid_klines`, `_HL_KLINES_MAX_LIMIT`, `_HL_MAX_KLINE_PAGES`), `static/js/split/078_v6_local_engine_client.js` (`DEEP_HISTORY_LIMIT`).
- Bug additionnel trouve et corrige dans la foulee: l'ancrage `start_time = now - limit*interval_ms` (et son equivalent `end_time` cote Hyperliquid) recalculait `now` a chaque requete, donc `cache_key`/`current_start` differait a chaque appel et une pagination 50 pages ne touchait JAMAIS le cache (24s a chaque fois, meme back-to-back). Fix: arrondir `now`/`end_time` au bucket `_KLINES_CACHE_TTL` (300s) / `_HL_TTLS["candles"]` (20s) avant de calculer l'ancre, pour que les requetes repetees dans la fenetre de cache reutilisent le meme `cache_key`.

### LESSON-20260611-06 - Deep history 1m x100 et symboles crypto futurs

- Contexte: L'utilisateur a demande environ 100x plus d'historique en 1m, et une implementation qui ne soit pas limitee a BTC pour les futures cryptos, que la source soit Binance ou Hyperliquid.
- Regle: Les limites d'historique chart doivent etre centralisees par timeframe, pas codees en dur a `1000`, `500`, ou une seule constante globale. Palier courant: `1m=100000`, `3m/5m=60000`, `15m/30m=40000`, `1h..4h=20000`, timeframes plus hautes reduites. Appliquer la meme table sur chart classique, widget BTC, prefill orderflow et deep REST fallback.
- Regle de prevention: Ne jamais remettre une whitelist Binance statique (`BTCUSDT`, `ETHUSDT`, `SOLUSDT`) pour les klines publiques. Utiliser une validation de format de paire crypto (`USDT/USDC/FDUSD/BTC/ETH/BNB`) et laisser l'exchange repondre si le symbole n'existe pas. Cote Hyperliquid, augmenter `_HL_MAX_KLINE_PAGES` quand `_HL_KLINES_MAX_LIMIT` augmente, sinon le backend plafonne silencieusement avant le nouveau `limit`.
- Test de non-regression: `build.py`, `node --check static/app.js`, suite unittest; verifier que `/api/market/klines?symbol=DOGEUSDT&interval=1m&limit=100000&soft=1` n'est pas rejete par validation locale et que les URLs frontend demandent `limit=100000` en `1m`.
- Fichiers a surveiller: `app_parts/23_routes_market.py`, `app_parts/24_market_history_cache.py`, `app_parts/25_routes_hyperliquid.py`, `static/js/split/060_btc_chart_widget.js`, `static/js/split/062_chart_page.js`, `static/js/split/073_v6_orderflow_layout.js`, `static/js/split/078_v6_local_engine_client.js`.

### LESSON-20260611-07 - Max history affichable pour CVD, DOM, Tape, Heatmap et Footprint

- Contexte: Apres le passage des bougies a 100k, l'utilisateur a precise que le CVD, le DOM et les panneaux orderflow doivent aussi afficher le maximum d'historique disponible.
- Regle: Quand on augmente l'horizon chart, relever aussi les buffers et clamps orderflow: trades/tape, CVD buckets par intervalle, depth history, heatmap frames, footprint candles, REST footprint limit et prefill aggTrades. Les defaults et validations doivent converger sur le meme plafond affichable (`100000`) au lieu de garder des restes `500`, `3000`, `5000` ou `10000`.
- Regle de prevention: Le DOM depth est borne par les limites reelles de l'exchange (Binance spot 5000, futures 1000), mais l'historique/affichage local ne doit pas recouper plus court. Les reglages persistants (`079`), le layout (`073`), les panneaux (`074`/`092`), l'inspector (`081`) et le client engine (`078`) doivent etre modifies ensemble, sinon un ancien clamp reprend la main.
- Test de non-regression: `tests/test_market_aggtrades.py` doit attendre `maxTrades=100000`; `tests/test_orderflow_trade_prefill.py` doit attendre `restTradePrefillLimit=100000`; `build.py` puis `node --check static/app.js`.
- Fichiers a surveiller: `app_parts/23_routes_market.py`, `static/js/split/070_v6_orderflow_contract.js`, `static/js/split/072_v6_orderflow_helpers.js`, `static/js/split/073_v6_orderflow_layout.js`, `static/js/split/074_v6_tape_panel.js`, `static/js/split/078_v6_local_engine_client.js`, `static/js/split/079_v6_orderflow_settings.js`, `static/js/split/081_v6_orderflow_inspector.js`, `static/js/split/092_v6_panel_settings.js`, `services/market-go/internal/ws/trade_store.go`.

### BUG-20260612-01 - [RESOLU] Fragmentation CSS orderflow v6 : 4 fichiers en guerre + colonnes DOM hardcodees

- Symptome: Le DOM panel orderflow avait 4 fichiers CSS (070, 071, 072, 073) qui se battaient avec des regles contradictoires pour les memes selecteurs. Les colonnes etaient definies en CSS avec des pourcentages fixes (bid=18%, price=16%, etc.) impossibles a reconfigurer. La colonne rail a 72px en 071 necessitait un override. Les reglages panel settings ne synchro pas les colonnes affichees.

- Cause racine: Accumulation de CSS incrementale sans consolidation. Chaque fichier ajoute des regles qui se sur-ecrivent sans strategie de cascade. Les colonnes du DOM etaient definies en dur dans le CSS ET dans le JS (COLUMN_WEIGHTS), creant 2 sources de verite. Quand le mode d'affichage change (colonnes masquees/reordonnees), le CSS force toujours les memes largeurs.

- Regle de prevention:
  1. **Une seule source de verite pour le layout des colonnes**: `COLUMN_DEFS` dans le JS (075_v6_dom_panel.js), registre unique avec `{weight, label, render(ctx)}`. Le CSS ne definit plus les largeurs individuelles — le JS les calcule en `%` depuis les weights et les injecte en inline `style="width:..."`.
  2. **Un seul fichier CSS visuel pour un design system**: `084_v6_exocharts_clean.css` est le seul fichier CSS actif pour l'UI orderflow. Les fichiers obsoletes (072, 073, 083) sont neutralises (regles videes ou garde-fous) mais conserves dans le build pour eviter les ruptures de numerotation.
  3. **070 reste en @layer** pour les tokens et resets, 071 garde uniquement les regles de layout shell (pas de visuel).
  4. **Ne jamais hardcoder des largeurs de colonnes en CSS quand elles dependent d'un etat dynamique** (colonnes activees/desactivees via settings). Toujours les calculer dans le JS et les passer en inline.
