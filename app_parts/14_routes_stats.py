...
def stats():
    ...
    EMPTY = {
        ...
        "plan_matrix": {}, "by_plan_error": {}, "plan_summary": {},
    ...
    by_plan_error = _bucket_multi(trades, lambda t: t.get("plan_errors"))

    plan_matrix = {
        "in_plan_win": {"label": "Dans le plan - gagnant", "count": 0, "pnl": 0},
        "in_plan_loss": {"label": "Dans le plan - perdant", "count": 0, "pnl": 0},
        "out_of_plan_win": {"label": "Hors plan - gagnant", "count": 0, "pnl": 0},
        "out_of_plan_loss": {"label": "Hors plan - perdant", "count": 0, "pnl": 0},
        "incomplete": {"label": "Plan incomplet", "count": 0, "pnl": 0},
        "unknown": {"label": "Plan inconnu", "count": 0, "pnl": 0},
    }
    scored = []
    in_plan_count = out_plan_count = incomplete_count = unknown_count = 0
    for t in trades:
        alignment = t.get("plan_alignment") or "unknown"
        win_state = t.get("_is_win_eff")
        pnl_val = t.get("_pnl_eff", t.get("pnl")) or 0
        if alignment == "in_plan":
            in_plan_count += 1
            key = "in_plan_win" if win_state == 1 else "in_plan_loss" if win_state == 0 else "incomplete"
        elif alignment == "out_of_plan":
            out_plan_count += 1
            key = "out_of_plan_win" if win_state == 1 else "out_of_plan_loss" if win_state == 0 else "incomplete"
            ...
            unknown_count += 1
            key = "unknown"
        plan_matrix[key]["count"] += 1
        plan_matrix[key]["pnl"] += pnl_val
        if t.get("plan_score") is not None:
            scored.append(float(t.get("plan_score") or 0))
    plan_known = in_plan_count + out_plan_count
    plan_summary = {
        "avg_score": (sum(scored) / len(scored)) if scored else 0,
        "in_plan": in_plan_count,
        "out_of_plan": out_plan_count,
        "incomplete": incomplete_count,
        "unknown": unknown_count,
        "in_plan_rate": (in_plan_count / plan_known * 100) if plan_known else 0,
        "out_of_plan_rate": (out_plan_count / plan_known * 100) if plan_known else 0,
        "scored": len(scored),
    }
    ...
    if plan_summary.get("scored", 0) >= 3:
        insights.append({
            "kind": "plan_discipline",
            "color": "lime" if plan_summary.get("in_plan_rate", 0) >= 70 else "amber",
            "icon": "compass",
            "title": "Discipline PO3",
            "body": f"{plan_summary.get('in_plan_rate', 0):.0f}% de trades dans le plan sur {plan_summary.get('scored', 0)} trades classes",
        })
